...
/**
 * Manages the versions of the message used by {@link SymmetricCryptAndHmacWrap} and
 * {@link SymmetricCryptAndHmacUnwrap}
 */
public class SymmetricCryptAndHmacMessage {
  ...
  public static List<MessageField> getVersion(int version, Key key)
    ...
    SymmetricCryptAndHmacKey symmetricCryptAndHmacKey = (SymmetricCryptAndHmacKey)key;

    List<MessageField> fields = new ArrayList<MessageField>();
    fields.add(new MessageField("msgVersion", INTEGER, 1));
    fields.add(new MessageField("iv", BYTES_FIXED, symmetricCryptAndHmacKey.getIvSize() / 8));
    fields.add(new MessageField("ciphertext", BYTES_VARIABLE, 1));
    fields.add(new MessageField("hmac", BYTES_FIXED, symmetricCryptAndHmacKey.getHmacSize() / 8));
