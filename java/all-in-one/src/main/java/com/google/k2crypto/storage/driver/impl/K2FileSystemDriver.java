 ...
 */
@DriverInfo(id = K2FileSystemDriver.NATIVE_SCHEME, name = "K2 Native File-System Driver",
    version = "0.1")
public class K2FileSystemDriver implements Driver, ReadableDriver, WritableDriver {
   ...
   */
  static final int MAX_FILENAME_LENGTH = 255 - (FILE_EXTENSION.length() + 1) - TEMP_PREFIX.length()
  ...
  private static final Pattern FILENAME_REGEX = Pattern
      .compile("^(?![\\p{Z}\\~\\.])" + "[^" + FILENAME_EXCLUSIONS + "]{1," + MAX_FILENAME_LENGTH
          + "}" + "(?<![\\p{Z}\\.])" + "\\." + Pattern.quote(FILE_EXTENSION) + '$');

  // Regex for checking if the address path already has the file extension.
  private static final Pattern EXTENSION_REGEX =
      Pattern.compile("\\." + Pattern.quote(FILE_EXTENSION) + '$', Pattern.CASE_INSENSITIVE);
  ...
  public URI open(final URI address) throws IllegalAddressException {
    ...
    if (scheme == null || scheme.equalsIgnoreCase(FILE_SCHEME)) {
      ...
      // Unrecognized scheme
      throw new IllegalAddressException(address, IllegalAddressException.Reason.INVALID_SCHEME,
          null);
    }
    ...
    if (!EXTENSION_REGEX.matcher(path).find()) {
      if (mustHaveExtension) {
        throw new IllegalAddressException(address, IllegalAddressException.Reason.INVALID_PATH,
            null);
      }
      // Append if missing
    ...
    try {
      ...
      final File tmpA = new File(parent, TEMP_PREFIX + filename + TEMP_A_EXTENSION);
      final File tmpB = new File(parent, TEMP_PREFIX + filename + TEMP_B_EXTENSION);
    ...
    throw new IllegalAddressException(address, IllegalAddressException.Reason.INVALID_PATH, null);
  }
  ...
  public void save(Key key) throws StoreException {
    ...
    if (keyFile.isFile()) {
      ...
      if (!keyFile.renameTo(other) || !target.renameTo(keyFile)) {
        throw new StoreIOException(StoreIOException.Reason.WRITE_ERROR);
      }
  ...
  private static byte[] serializeKey(Key key) throws StoreIOException {
    try {
      ...
      throw new StoreIOException(StoreIOException.Reason.SERIALIZATION_ERROR, ex);
    }
  }
  ...
  private void writeKey(byte[] keyBytes, File file) throws StoreIOException {
    ...
    if (exception != null || file.length() != keyBytes.length) {
      file.delete();
      throw new StoreIOException(StoreIOException.Reason.WRITE_ERROR, exception);
    }
  }
  ...
  public Key load() throws StoreException {
    ...
    File[] candidates = isFormerMoreReadable(tempFileA, tempFileB)
        ? new File[] {keyFile, tempFileA, tempFileB} : new File[] {keyFile, tempFileB, tempFileA};

    // Attempt to read each file and return the first successfully parsed Key
    ExtensionRegistry registry = context.getKeyVersionRegistry().getProtoExtensions();
    ...
    for (File file : candidates) {
      try {
        ...
        if (ioException == null || ex.getReason().compareTo(ioException.getReason()) < 0) {
          ioException = ex;
        }
      }
    }
   ...
   */
  private Key readKey(File file, ExtensionRegistry registry) throws StoreIOException {
    ...
    try {
      ...
      throw new StoreIOException(StoreIOException.Reason.DESERIALIZATION_ERROR, ex);
    } catch (UnregisteredKeyVersionException ex) {
      throw new StoreIOException(StoreIOException.Reason.UNREGISTERED_KEY_VERSION, ex);
    } finally {
      try {
        in.close();
      } catch (Exception ex) {
