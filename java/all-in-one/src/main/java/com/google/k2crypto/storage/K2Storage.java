...
public class K2Storage {
  ...
  private final Map<String, InstalledDriver> drivers = new LinkedHashMap<String, InstalledDriver>();
  ...
  public Store open(String address)
    ...
    try {
      ...
      throw new IllegalAddressException(address, IllegalAddressException.Reason.INVALID_URI, ex);
    }
  }
  ...
  public List<InstalledDriver> getInstalledDrivers() {
    synchronized (drivers) {
      ...
      if (list == null) {
        list = Collections.unmodifiableList(new ArrayList<InstalledDriver>(drivers.values()));
