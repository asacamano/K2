...
public class KeyVersionRegistryTest {
   ...
   */
  @Test
  public final void testRegisterBadKeyVersion() {
   ...
   */
  private void checkNotRegistered(KeyVersionRegistry registry, Type type, FieldDescriptor fd) {
    assertNull(registry.getRegistration(type));
    assertFalse(registry.isRegistered(type));
    assertNull(registry.getProtoExtensions().findExtensionByName(fd.getFullName()));
    try {
   ...
   * @param fd Field descriptor of the proto-extension associated with the type.
   */
  private void checkRegistered(KeyVersionRegistry registry, RegisteredKeyVersion rkv,
    ...
    assertEquals(fd,
        registry.getProtoExtensions().findExtensionByName(fd.getFullName()).descriptor);
    assertEquals(rkv.getKeyVersionClass(), registry.newBuilder(type).build().getClass());
  }
   ...
   */
  @Test
  public final void testOneKeyVersion() throws K2Exception {
    KeyVersionRegistry registry = new KeyVersionRegistry(context);
   ...
   */
  @Test
  public final void testSeveralKeyVersions() throws K2Exception {
  ...
  // Test "data" for the above
  @KeyVersionInfo(type = Type.AES, proto = AesKeyVersionProto.class)
  ...
  // Test "data" for the above
  @KeyVersionInfo(type = Type.HMAC, proto = HmacKeyVersionProto.class)
