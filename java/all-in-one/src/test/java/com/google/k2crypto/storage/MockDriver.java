...
public abstract class MockDriver implements Driver {
  ...
  public Key load() throws StoreException {
    ...
    if (storedKey != null) {
      if (storedKeyWrapper == null) {
        if (wrapKey != null) {
          throw new StoreIOException(StoreIOException.Reason.WRAP_KEY_UNNECESSARY);
        }
      } else if (!storedKeyWrapper.equals(wrapKey)) {
        throw new StoreIOException(wrapKey == null ? StoreIOException.Reason.WRAP_KEY_REQUIRED
  ...
  public static class ReadOnly extends MockDriver implements ReadableDriver, WrappingDriver {
  ...
  public static class WriteOnly extends MockDriver implements WritableDriver, WrappingDriver {
  ...
  public static class NoWrap extends MockDriver implements ReadableDriver, WritableDriver {
  ...
  public static class AcceptAll extends MockDriver
    ...
    public URI open(URI address) throws IllegalAddressException, StoreException {
      try {
        return super.open(
            new URI(getInfo().id(), address.getSchemeSpecificPart(), address.getFragment())
                .normalize());
  ...
  public static class AcceptFile extends MockDriver
    ...
    public URI open(URI address) throws IllegalAddressException, StoreException {
      ...
      try {
        if (getInfo().id().equalsIgnoreCase(scheme) || "file".equalsIgnoreCase(scheme)) {
          return super.open(
              new URI(getInfo().id(), address.getSchemeSpecificPart(), address.getFragment())
                  .normalize());
        }
