...
public class AddressUtilitiesTest {
   ...
   */
  @Test
  public final void testEncodeConvenience() {
    // Check empty string
    ...
    final String[] input = {"", " ", "%", "%G", "%g", "%0G", "%0g"};
    final String[] encoded = {"", "%20", "%25", "%25G", "%25g", "%250G", "%250g"};
    ...
    for (String id : identities) {
      for (int i = 0; i < input.length; ++i) {
        for (int j = 0; j < input.length; ++j) {
          assertEquals(encoded[i] + id + encoded[j],
   ...
   */
  @Test
  public final void testDecodeUnreserved() {
    // Check decoding of all extreme unreserved cases
    assertEquals("--..__~~09", AddressUtilities.decodeUnreserved("%2D%2d%2E%2e%5F%5f%7E%7e%30%39"));
    assertEquals("AIJJOOPYZZ", AddressUtilities.decodeUnreserved("%41%49%4A%4a%4F%4f%50%59%5A%5a"));
    assertEquals("aijjoopyzz", AddressUtilities.decodeUnreserved("%61%69%6A%6a%6F%6f%70%79%7A%7a"));
    ...
    assertEquals(URI.create("k2://h%2F/%3f%23/../~/./key"),
        AddressUtilities.decodeUnreserved(URI.create("k2://%68%2F/%3f%23/%2e%2E/%7e/%2E/%6bey")));
  }
   ...
   */
  @Test
  public final void testCheckNoAuthority() {
    // Acceptance cases
    for (String address : new String[] {"k2:///p?q#f", "/p?q#f", "///p", ""}) {
      try {
    ...
    // Rejection cases
    for (String address : new String[] {"k2://u@h:1", "//:1", "//u@", "//h"}) {
      try {
        AddressUtilities.checkNoAuthority(URI.create(address));
        fail("Should reject " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.AUTHORITY_UNSUPPORTED, expected.getReason());
      }
    }
  }
   ...
   */
  @Test
  public final void testCheckNoUser() {
    // Acceptance cases
    for (String address : new String[] {"k2://h:1/p?q#f", "//@h:1", "@h", ""}) {
      try {
    ...
    // Rejection cases
    for (String address : new String[] {"k2://u@h:1", "//u@h"
      ...
      try {
        ...
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.USER_UNSUPPORTED, expected.getReason());
      }
    }
  }
   ...
   */
  @Test
  public final void testCheckNoHostPort() {
    // Acceptance cases
    for (String address : new String[] {"k2://u@/p?q#f", "//:0", "//:", "///p", ""}) {
      try {
    ...
    // Rejection cases
    for (String address : new String[] {"k2://h:1", "//h:0", "//h:", "//h"
      ...
      try {
        ...
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.HOST_PORT_UNSUPPORTED, expected.getReason());
      }
    }
  }
   ...
   */
  @Test
  public final void testCheckNoPath() {
    // Acceptance cases
    for (String address : new String[] {"k2://u@h:1?q#f", "//h:1", "?q#f", ""}) {
      try {
    ...
    // Rejection cases
    for (String address : new String[] {"k2://h:1/", "///", "/", ".", "..", "~"}) {
      try {
        AddressUtilities.checkNoPath(URI.create(address));
        fail("Should reject: " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.PATH_UNSUPPORTED, expected.getReason());
      }
    }
  }
   ...
   */
  @Test
  public final void testCheckNoQuery() {
    // Acceptance cases
    for (String address : new String[] {"k2://u@h:1/p#f", "//u@h:1/p?#f", "?#f", "?", ""}) {
      try {
    ...
    // Rejection cases
    for (String address : new String[] {"k2:/p?q", "//h:1/?q", "?q"}) {
      try {
        AddressUtilities.checkNoQuery(URI.create(address));
        fail("Should reject " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.QUERY_UNSUPPORTED, expected.getReason());
      }
    }
  }
   ...
   */
  @Test
  public final void testCheckNoFragment() {
    // Acceptance cases
    for (String address : new String[] {"k2://u@h:1/p?q", "//u@h:1/p?q#", "?q#", "#", ""}) {
      try {
    ...
    // Rejection cases
    for (String address : new String[] {"k2:/p#f", "//h:1/?#f", "#f"}) {
      try {
        AddressUtilities.checkNoFragment(URI.create(address));
        fail("Should reject " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.FRAGMENT_UNSUPPORTED, expected.getReason());
      }
    }
  }
   ...
   */
  @Test
  public final void testExtractHost() {
    // Acceptance cases
    ...
    // Rejection cases
    for (String address : new String[] {"k2:p?q#f", "p?q#f", "h", ""}) {
      try {
        AddressUtilities.extractHost(URI.create(address));
        fail("Should reject " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.MISSING_HOST_PORT, expected.getReason());
      }
    }
  }
  ...
  private static void checkExtractHost(String expected, String address) {
    try {
      assertEquals(expected, AddressUtilities.extractHost(URI.create(address)));
    } catch (IllegalAddressException ex) {
      throw new RuntimeException("Address is acceptable: " + address, ex);
    }
  }
   ...
   */
  @Test
  public final void testExtractRawPath() {
    // Acceptance cases
    ...
    for (String address : new String[] {"k2://u@h:1?q#f", "//h:1", "?q#f", ""}) {
      try {
        ...
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.MISSING_PATH, expected.getReason());
      }
    }
  }
  ...
  private static void checkExtractRawPath(String expected, String address) {
    try {
      assertEquals(expected, AddressUtilities.extractRawPath(URI.create(address)));
    } catch (IllegalAddressException ex) {
      throw new RuntimeException("Address is acceptable: " + address, ex);
    }
  }
   ...
   */
  @Test
  public final void testExtractRawQuery() {
    // Acceptance cases
    ...
    // Rejection cases
    for (String address : new String[] {"k2://u@h:1/p?#f", "//h:1/p", "#f", "?", "%3Fq", ""}) {
      try {
        AddressUtilities.extractRawQuery(URI.create(address));
        fail("Should reject " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.MISSING_QUERY, expected.getReason());
      }
    }
  }
  ...
  private static void checkExtractRawQuery(String expected, String address) {
    try {
      assertEquals(expected, AddressUtilities.extractRawQuery(URI.create(address)));
    } catch (IllegalAddressException ex) {
      throw new RuntimeException("Address is acceptable: " + address, ex);
    }
  }
   ...
   */
  @Test
  public final void testExtractFragment() {
    // Acceptance cases
    ...
    // Rejection cases
    for (String address : new String[] {"k2://u@h:1/p?q#", "//h:1/p?q", "#", "%23f", ""}) {
      try {
        AddressUtilities.extractFragment(URI.create(address));
        fail("Should reject " + address);
      } catch (IllegalAddressException expected) {
        assertEquals(address, expected.getAddress());
        assertEquals(IllegalAddressException.Reason.MISSING_FRAGMENT, expected.getReason());
      }
    }
  }
  ...
  private static void checkExtractFragment(String expected, String address) {
    try {
      assertEquals(expected, AddressUtilities.extractFragment(URI.create(address)));
    } catch (IllegalAddressException ex) {
      throw new RuntimeException("Address is acceptable: " + address, ex);
    }
  }
