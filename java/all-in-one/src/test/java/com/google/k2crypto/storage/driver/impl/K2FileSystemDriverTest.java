 ...
 */
public class K2FileSystemDriverTest extends FileBasedDriverTest<K2FileSystemDriver> {
   ...
   */
  @Test
  public final void testRejectBadAddresses() {
    ...
    checkRejectAddress(FILE_PREFIX + "//user@localhost:80/path",
        IllegalAddressException.Reason.AUTHORITY_UNSUPPORTED);
    checkRejectAddress(FILE_PREFIX + "/path?que", IllegalAddressException.Reason.QUERY_UNSUPPORTED);
    checkRejectAddress(FILE_PREFIX + "/path#frag",
        IllegalAddressException.Reason.FRAGMENT_UNSUPPORTED);

    // Test invalid schemes
    checkRejectAddress("k3:/path", IllegalAddressException.Reason.INVALID_SCHEME);
    checkRejectAddress("keyczar:/path", IllegalAddressException.Reason.INVALID_SCHEME);

    // Test generic schemes without the proper lowercase ".k2k" extension
    checkRejectAddress(testingDirPath + "myfile", IllegalAddressException.Reason.INVALID_PATH);
    checkRejectAddress(testingDirPath + "myfile" + NATIVE_POSTFIX.toUpperCase(),
        IllegalAddressException.Reason.INVALID_PATH);
    checkRejectAddress(FILE_PREFIX + testingDirPath + "myfile",
        IllegalAddressException.Reason.INVALID_PATH);
    checkRejectAddress(FILE_PREFIX + testingDirPath + "myfile" + NATIVE_POSTFIX.toUpperCase(),
        IllegalAddressException.Reason.INVALID_PATH);
    checkRejectAddress(FILE_PREFIX + testingDirPath + "myfile%2e%6B%32%4B",
        IllegalAddressException.Reason.INVALID_PATH);

    // Test no path
    checkRejectAddress(NATIVE_PREFIX + "?", IllegalAddressException.Reason.MISSING_PATH);

    // Test relative path going beyond the logical URI root
    checkRejectAddress("/../my%20key", IllegalAddressException.Reason.INVALID_PATH);

    final String testingAddress = NATIVE_PREFIX + testingDirPath;

    // Test all illegal filename characters
    for (char illegal : new char[] {'\0', '\n', '\r', '\t', '\f', '\b', '\u007F', '\\', '/', '*',
        '?', '|', '<', '>', ':', ';', '"'}) {
      ...
      checkRejectAddress(testingAddress + 'A' + encoded + 'Z',
          IllegalAddressException.Reason.INVALID_PATH);
      checkRejectAddress(testingAddress + encoded, IllegalAddressException.Reason.INVALID_PATH);
    }
   ...
   */
  @Test
  public final void testFilenameLength() throws K2Exception {
    ...
    try {
      driver.open(URI.create(testingAddress + generateString(MAX_FILENAME_LENGTH)));
    } finally {
      driver.close();
    }
  }
   ...
   */
  @Test
  public final void testRejectBadFileLocation() throws IOException {
    ...
    try {
      ...
      checkRejectAddress(NATIVE_PREFIX + parent.toURI().getRawPath() + "/keys",
          IllegalAddressException.Reason.INVALID_PATH);
    } finally {
      parent.delete();
    }
    ...
    try {
      ...
      for (File f : files) {
        assertTrue(f.mkdir());
        checkRejectAddress(mainAddress, IllegalAddressException.Reason.INVALID_PATH);
        assertTrue(f.delete());
      }
    } finally {
      deleteAll(files);
   ...
   */
  @Test
  public final void testAddressNormalization() throws K2Exception {
    ...
    checkNormalization(expected, absTestingAddress + "anything/./something/.././../" + filename);
    checkNormalization(expected, absTestingAddress + "/././" + filename + NATIVE_POSTFIX);

    // Test the above with absolute paths (i.e. no k2: scheme)
    checkNormalization(expected, absTestingPath + filename + NATIVE_POSTFIX);
    checkNormalization(expected, absTestingPath + "/./something/../" + filename + NATIVE_POSTFIX);

    // Test the above with relative paths
    checkNormalization(expected, relTestingPath + filename + NATIVE_POSTFIX);
    checkNormalization(expected, relTestingPath + "/anything/../" + filename + NATIVE_POSTFIX);
  }
   ...
   */
  @Test
  public final void testSaveLoadErase() throws K2Exception {
    File[] files = generateFileTriple(getTestingDir());
   ...
   */
  @Test
  public final void testRecovery() throws K2Exception, IOException {
    File[] files = generateFileTriple(getTestingDir());
    URI address = files[0].toURI().normalize();
    deleteAllOnExit(files);
    ...
    try {
      // Open and save a key
      assertEquals(NATIVE_PREFIX + address.getSchemeSpecificPart(),
   ...
   */
  @Test
  public final void testRecoveryPrecedence() throws K2Exception, IOException {
    final File keyFile = generateFile(getTestingDir(), "key", NATIVE_POSTFIX);
    final File emptyFile = generateFile(getTestingDir(), "empty", NATIVE_POSTFIX);
    File[] files = generateFileTriple(getTestingDir());
    URI address = files[0].toURI().normalize();
   ...
   */
  @Test
  public final void testLoadEraseCorrupted() throws K2Exception, IOException {
    File[] files = generateFileTriple(getTestingDir());
    URI address = files[0].toURI().normalize();
    deleteAllOnExit(files);
