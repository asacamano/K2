...
public final class StorageExample {
  ...
  private static final File WORKING_DIRECTORY = new File("./build/tmp/");

  // File pointing to a native K2 key file
  private static final File NATIVE_STORE = new File(WORKING_DIRECTORY, "example mykeys.k2k");

  // File pointing to a SQLite database file
  private static final File SQLITE_DATABASE = new File(WORKING_DIRECTORY, "example sqlite.db");
  ...
  public static void main(String[] args) throws K2Exception {
    ...
    try {
      ...
      store = storage.open("sqlite:" + SQLITE_DATABASE.toURI().getRawPath() + "#samekey");
