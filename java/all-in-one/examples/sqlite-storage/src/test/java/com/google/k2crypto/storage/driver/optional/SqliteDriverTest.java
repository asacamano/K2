...
public class SqliteDriverTest extends FileBasedDriverTest<SqliteDriver> {
  ...
  public final void testRejectBadAddresses() {
    ...
    checkRejectAddress(ADDRESS_PREFIX + "//host/database" + GENERIC_KEY_ID,
        IllegalAddressException.Reason.AUTHORITY_UNSUPPORTED);
    checkRejectAddress(ADDRESS_PREFIX + "//user@localhost:80/database" + GENERIC_KEY_ID,
        IllegalAddressException.Reason.AUTHORITY_UNSUPPORTED);
    checkRejectAddress(ADDRESS_PREFIX + "/database?password=1234" + GENERIC_KEY_ID,
        IllegalAddressException.Reason.QUERY_UNSUPPORTED);

    // Test invalid schemes
    checkRejectAddress("k2:/database" + GENERIC_KEY_ID,
        IllegalAddressException.Reason.INVALID_SCHEME);
    checkRejectAddress("file:/database" + GENERIC_KEY_ID,
        IllegalAddressException.Reason.INVALID_SCHEME);
    checkRejectAddress("/database" + GENERIC_KEY_ID, IllegalAddressException.Reason.INVALID_SCHEME);

    // Test no database path
    checkRejectAddress(ADDRESS_PREFIX + "host" + GENERIC_KEY_ID,
    ...
    for (char illegal : new char[] {'\0', '\n', '\r', '\t', '\f', '\b', '\u007F', '\\', '/', '*',
      ...
      checkRejectAddress(testingAddress + 'A' + encoded + 'Z' + GENERIC_KEY_ID,
          IllegalAddressException.Reason.INVALID_PATH);
      checkRejectAddress(testingAddress + encoded + GENERIC_KEY_ID,
          IllegalAddressException.Reason.INVALID_PATH);
    }

    // Test illegal database filename prefixes
    for (String illegalPrefix : new String[] {"~", ".", "%20"}) {
      checkRejectAddress(testingAddress + illegalPrefix + "abc" + GENERIC_KEY_ID,
          IllegalAddressException.Reason.INVALID_PATH);
    }

    // Test illegal database filename postfixes
    for (String illegalPostfix : new String[] {".", "%20"}) {
      checkRejectAddress(testingAddress + "abc" + illegalPostfix + GENERIC_KEY_ID,
          IllegalAddressException.Reason.INVALID_PATH);
    }

    // Test no fragment (key identifier)
    checkRejectAddress(ADDRESS_PREFIX + "/database",
    ...
    try {
      ...
      for (char illegal : new char[] {'\0', '\n', '\r', '\t', '\f', '\b', '\u007F',}) {
        String encoded = String.format("%%%02X", (int) illegal);
        assertEquals(3, encoded.length()); // sanity check
        checkRejectAddress(generateAddress(db, 'A' + encoded + 'Z'),
            IllegalAddressException.Reason.INVALID_FRAGMENT);
        checkRejectAddress(generateAddress(db, encoded),
            IllegalAddressException.Reason.INVALID_FRAGMENT);
      }

      // Test no spaces at start/end of key identifier
      checkRejectAddress(generateAddress(db, "%20key"),
          IllegalAddressException.Reason.INVALID_FRAGMENT);
      checkRejectAddress(generateAddress(db, "key%20"),
          IllegalAddressException.Reason.INVALID_FRAGMENT);
    } finally {
      db.delete();
    }
   ...
   */
  @Test
  public final void testKeyIdentifierLength() throws K2Exception {
    File db = generateTempDatabase();
    try {
      // Test key identifier that is one character too long
      String oneCharTooLongId = generateString(MAX_KEY_ID_LENGTH + 1);
      checkRejectAddress(generateAddress(db, oneCharTooLongId),
          IllegalAddressException.Reason.INVALID_FRAGMENT);
   ...
   */
  @Test
  public final void testRejectBadDatabaseLocation() {
    ...
    if (roots != null) {
      ...
      for (File root : roots) {
        checkRejectAddress(ADDRESS_PREFIX + root.toURI().getRawPath() + GENERIC_KEY_ID,
            IllegalAddressException.Reason.INVALID_PATH);
      }
    }
    ...
    try {
      db.delete();
      assertTrue(db.mkdir());
      checkRejectAddress(ADDRESS_PREFIX + db.toURI().getRawPath() + GENERIC_KEY_ID,
          IllegalAddressException.Reason.INVALID_PATH);
    } finally {
      db.delete();
    }
    ...
    try {
      checkRejectAddress(ADDRESS_PREFIX + parent.toURI().getRawPath() + "/k" + GENERIC_KEY_ID,
          IllegalAddressException.Reason.INVALID_PATH);
    } finally {
      parent.delete();
    }
  ...
  public final void testAddressNormalization() throws K2Exception {
    ...
    try {
      ...
      final String absTestingPath = new File("").toURI()
          .resolve(db.getParentFile().toURI().getRawPath()).normalize().getRawPath();
      final String absTestingAddress = ADDRESS_PREFIX + absTestingPath;

      checkNormalization(expected, absTestingAddress + filename + GENERIC_KEY_ID);
      checkNormalization(expected, absTestingAddress + filename + '?' + GENERIC_KEY_ID);
      checkNormalization(expected, absTestingAddress + "/././" + filename + GENERIC_KEY_ID);
  ...
  private static URI generateAddress(File database, String keyName) {
    return URI.create(ADDRESS_PREFIX + database.toURI().normalize().getRawPath() + '#' + keyName);
  }
}
